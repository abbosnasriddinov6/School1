import React from 'react'
import '../../scss/pages/Price.scss'
const page = () => {
    return (
        <>
            <div className='Section7'>
                <div className="container">
                    <div className="section7">
                        <div className="narx">
                            <h1>Narxlarimiz</h1>
                        </div>
                        <div className="sinflar">
                            <div data-aos="fade-right"  className="ajral">
                                <div className='sinf1'>
                                    <h4>1-4 o'zbek sinflar uchun</h4>
                                </div>
                                <div className='qolgan'>
                                    <h4>Qolgan barcha sinflar uchun</h4>
                                </div>
                            </div>
                            <div data-aos="fade-left"  className="ajralma">
                                <div className='oshxona'>
                                    <h4>Oshxona to'lovi</h4>
                                </div>
                            </div>
                        </div>
                        <div className="pul">
                            <div data-aos="fade-right"  className='ajral2'>
                                <div className='million4'>
                                    <h1>4 110 000 soʻm</h1>
                                </div>
                                <div className='million45'>
                                    <h1>4 690 000 soʻm</h1>
                                </div>
                            </div>
                            <div data-aos="fade-left"  className='mahal'>
                                <div className='birxil'>
                                    <h3>1 mahal</h3>
                                    <h4>490 000 <span>so'm</span></h4>
                                </div>
                                <div className='birxil'>
                                    <h3>3 mahal</h3>
                                    <h4>790 000 <span>so'm</span></h4>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className="section77">
                        <div className="narx1">
                            <h1>Stipendiyalar</h1>
                        </div>
                        <div className='boshlab'>
                            <div className="div11">
                                <div className="boshlab1">
                                    <p>5-sinfdan boshlab har oylik moddiy qo'llab-quvvatlov o'quvchilarning mustaqil bo'lib o'sishlari, moliyaviy savodxonliklarini oshirishlari va maktabda arzonroq o'qishlariga yordam beradi.</p>
                                    <p className='yoqol'>Stipendiya o'quvchining shaxsiy kartasiga o'tkazib beriladi va uni o'quvchi o'zi hohlagancha tasarruf qilishi mumkin. IELTS va SAT sertifikatiga ega o'quvchilar <span>deyarli bepulga o'qishadi.</span></p>
                                </div>
                            </div>
                            <div className="div12"></div>
                        </div>

                    </div>
                </div>
            </div>
            <div className="Stipendiya2">
                <div className="container">
                    <div className="stipendiya2">
                        <div className='Sti'>
                            <h1>Stipendiyalar</h1>
                            <h3>Stipendiya har oy beriladi va har <span>chorakda tasdiqlanadi</span></h3>
                        </div>
                        <div className='tablega'>
                            <table>
                              <tr>
                                  <th>Baholar</th>
                                  <th>IELTS 6 / 1100-1200 SAT</th>
                                  <th>IELTS 6.5 / 1201-1300 SAT</th>
                                  <th>IELTS 7 / 1301 - 1400 SAT</th>
                                  <th>IELTS 7.5 / 1401 - 1500 SAT</th>
                                  <th>IELTS 8 / 1501 - 1550 SAT</th>
                                  <th>IELTS 8.5 / 1551 - 1600 SAT</th>
                              </tr>
                              <tr>
                                  <td>2</td>
                                  <td>0</td>
                                  <td>0</td>
                                  <td>0</td>
                                  <td>0</td>
                                  <td>0</td>
                                  <td>0</td>
                              </tr>
                              <tr>
                                  <td>3</td>
                                  <td>500,000</td>
                                  <td>700,000</td>
                                  <td>1,000,000</td>
                                  <td>1,300,000</td>
                                  <td>1,800,000</td>
                                  <td>2,300,000</td>
                              </tr>
                              <tr>
                                  <td>4</td>
                                  <td>700,000</td>
                                  <td>1,000,000	</td>
                                  <td>1,300,000</td>
                                  <td>1,800,000</td>
                                  <td>2,300,000</td>
                                  <td>2,800,000</td>
                              </tr>
                              <tr>
                                  <td>5</td>
                                  <td>1,000,000</td>
                                  <td>1,300,000</td>
                                  <td>1,800,000</td>
                                  <td>2,300,000</td>
                                  <td>2,800,000</td>
                                  <td>3,300,000</td>
                              </tr>

                          </table>
                         
                        </div>
                    </div>
                </div>
            </div>
        </>
    )
}

export default page